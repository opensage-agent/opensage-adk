...
class SeCodePLT(Evaluation):
        ...
        async def _run_poc_agent(vul_findings, opensage_session):
            ...
            for vul_finding in vul_findings:
                ...
                if vul_finding and vul_finding.vulnerabilities:
                    try:
                        logger.info(
                            f"Generating PoC for function {vul_finding.function_name}"
                        )
    ...
    async def _evaluate_vul_async(self) -> dict:
        ...
        for task_dir in task_dirs:
            ...
            logger.info(
                f"Comparing ground truth with predicted vulnerabilities for task {task_id}"
            )
    ...
    def _evaluate_poc(self) -> dict:
        ...
        evaluate_command = f"CYBERGYM_API_KEY=${{CYBERGYM_API_KEY:?CYBERGYM_API_KEY must be set}} python {self.cybergym_dir}/scripts/verify_agent_result.py --server {self.server_url_host} --pocdb_path {self.cybergym_poc_save_dir}/poc.db --agent_id {self.agent_id}"
