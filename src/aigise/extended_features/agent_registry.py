...
class CustomAgentBuilder(AgentBuilder):
    ...
    def __init__(
        self, agent_class: Type[BaseAgent], template: Optional[AgentTemplate] = None
    ):
...
class AgentRegistry:
    ...
    def create_agent(
        self, builder_name: str, instance_name: Optional[str] = None, **kwargs
    ) -> BaseAgent:
        """Create an agent instance using the specified builder."""
        if builder_name not in self._builders:
            raise ValueError(
                f"Unknown builder '{builder_name}'. Available: {list(self._builders.keys())}"
            )
    ...
    def create_from_template(
        self, template_name: str, instance_name: Optional[str] = None, **kwargs
    ) -> BaseAgent:
...
def register_agent_class(
    name: str, agent_class: Type[BaseAgent], template: Optional[AgentTemplate] = None
) -> None:
...
def create_agent(
    builder_name: str, instance_name: Optional[str] = None, **kwargs
) -> BaseAgent:
...
def create_agent_from_template(
    template_name: str, instance_name: Optional[str] = None, **kwargs
) -> BaseAgent:
