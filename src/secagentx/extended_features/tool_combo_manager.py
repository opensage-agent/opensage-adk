...
def delegate_to_parent(tool_context: ToolContext) -> str:
    ...
    if hasattr(parent_agent, '__class__') and parent_agent.__class__.__name__ == 'SequentialAgent':
...
class ToolCombo:
    def __init__(self, name: str, tool_sequences: List[Any], description: str = "", model: Union[str, BaseLlm] = '', return_history: bool = True):
        self.name = name
    ...
    def _wrap_tool_as_agent(self, tool: Any, idx: int, total_tools: int) -> LlmAgent:
        ...
        if not isinstance(tool, BaseTool) and not callable(tool):
            raise ValueError(f"Tool at index {idx} must be a BaseTool, LlmAgent, or callable function")
