...
MAX_WINDOW_EXPANSION_EDIT_CONFIRM = int(
    REGISTRY.get("MAX_WINDOW_EXPANSION_EDIT_CONFIRM", 0)
)
...
class WindowExpander:
    ...
    def _find_breakpoints(
        ...
        if direction == 1:
            ...
            if current_line == len(lines):
                ...
                return current_line
            iter_lines = range(
                current_line, 1 + min(current_line + max_added_lines, len(lines))
            )
        elif direction == -1:
            # up
            if current_line == 1:
                ...
                return current_line
            iter_lines = range(
                current_line, -1 + max(current_line - max_added_lines, 1), -1
            )
        else:
        ...
        for i_line in iter_lines:
            ...
            if self.suffix == ".py" and any(
                re.match(regex, line)
                for regex in [r"^\s*def\s+", r"^\s*class\s+", r"^\s*@"]
            ):
    ...
    def expand_window(
        ...
        new_stop = self._find_breakpoints(
            lines, stop, direction=1, max_added_lines=max_added_lines
        )
...
class EditTool:
    ...
    def create_file(self, path: Path, file_text: str):
        if not path.parent.exists():
            print(
                f"The parent directory {path.parent} does not exist. Please create it first."
            )
            sys.exit(21)
    ...
    def view(self, path: Path, view_range: Optional[List[int]] = None):
        ...
        if view_range:
            ...
            init_line, final_line = WindowExpander(suffix=path.suffix).expand_window(
                ...
                init_line,
                final_line,
                max_added_lines=MAX_WINDOW_EXPANSION_VIEW,
            )
            ...
            if (
                path.suffix == ".py"
                and len(file_content) > MAX_RESPONSE_LEN
                and USE_FILEMAP
            ):
                try:
    ...
    def str_replace(self, path: Path, old_str: str, new_str: Optional[str]):
        ...
        if post_edit_lint:
            ...
            replacement_window_start_line = (
                file_content.split(old_str)[0].count("\n") + 1
            )
            replacement_lines = len(new_str.split("\n"))
            replacement_window_end_line = (
                replacement_window_start_line + replacement_lines - 1
            )
            replacement_window = (
                replacement_window_start_line,
                replacement_window_end_line,
    ...
    def read_file(self, path: Path):
        ...
        for self._encoding, errors in encodings:
            ...
            print(
                f"Ran into UnicodeDecodeError {exception} while trying to read {path}"
            )
            sys.exit(19)
        return text
