...
class AgentLifecycleHook:
    ...
    async def on_agent_error(self, agent: BaseAgent, metadata: AgentMetadata, error: Exception) -> None:
...
class DynamicAgentManager:
    ...
    async def create_agent(self, config: Dict[str, Any], 
                          creator: Optional[str] = None,
                          persist: bool = True) -> tuple[str, BaseAgent]:
        ...
        try:
            ...
            if 'template' in config:
                ...
                agent_config = {k: v for k, v in config.items() if k not in ['type', 'template']}
                agent = self.registry.create_from_template(template_name, **agent_config)
            else:
    ...
    async def create_agent_simple(self, config: Dict[str, Any],
                                 creator: Optional[str] = None,
    ...
    async def clone_agent(self, source_id: str, updates: Optional[Dict[str, Any]] = None,
                         creator: Optional[str] = None, persist: bool = True) -> tuple[str, BaseAgent]:
    ...
    def list_agents(self, status: Optional[AgentStatus] = None,
                   creator: Optional[str] = None) -> List[AgentMetadata]:
                ...
                if metadata.status != AgentStatus.ERROR and metadata.config:
                    try:
                        ...
                        if 'template' in metadata.config:
                            ...
                            agent_config = {k: v for k, v in metadata.config.items() 
                                          if k not in ['type', 'template']}
                            agent = self.registry.create_from_template(template_name, **agent_config)
                        else:
                            agent_config = {k: v for k, v in metadata.config.items() if k != 'type'}
                            agent = self.registry.create_agent(agent_type, **agent_config)
