...
Use the launch script at `examples/opensage/run_opensage_grpo.sh`:
...
The script kills stale sglang and rpc processes before starting.

### Default Configuration

The YAML config (`examples/opensage/opensage_grpo_mt.yaml`) sets these defaults:
...
| `generation_kwargs` | `{extra_body: {chat_template_kwargs: {enable_thinking: true}}}` | Generic kwargs merged into every `chat.completions.create()` call. Any key-value pairs here pass through to the OpenAI-compatible API. |
...
Use turn files to debug tokenizer and parser issues; use `trajectory_full.json` to analyze agent behavior.
...
This change also removes an unused `os` import and a hard-coded `log_level` parameter.
...
The CodeQL sandbox is now enabled. A previous bug in `tool_normalization.py` stripped `__sandbox_requirements__` metadata during tool wrapping, which prevented `collect_sandbox_dependencies()` from detecting CodeQL and Joern requirements. The sandbox pruner then dropped CodeQL as unused even when the config listed it. The fix restores the metadata during wrapping.

## 4. Known Issues

### Race Condition on Shared `self.model` in SeCodePLT (Fixed)

**Symptom:** When running with `group_size >= 2` (multiple concurrent rollout episodes), the error `"No interaction in cache to set reward for"` appears. Rewards misroute between episodes.
...
**Fix:** The mutation `self.model = task.model` now becomes a local `model_to_use` variable. A `model` parameter threads explicitly through `_detect_vulnerability_with_retry` and `_generate_poc_with_retry`, eliminating the shared mutable state.

### `__sandbox_requirements__` Lost During Tool Wrapping (Fixed)

**Symptom:** `collect_sandbox_dependencies()` only finds `{'neo4j', 'main'}` even though tools like `search_function` have `@requires_sandbox("neo4j", "codeql", "joern")`. The CodeQL sandbox is pruned as unused, and call-graph tools return empty results.

**Root cause:** `_make_safe_dict_callable()` in `tool_normalization.py` wraps tool functions but only copies 5 attributes. `__sandbox_requirements__` is not in the copy list, so the wrapper drops the metadata.
...
The CodeQL sandbox had two known initialization failures. Recent versions resolve these, but they may resurface:
...
The CodeQL initializer cannot connect to the Neo4j instance when inserting call-graph results. This appears to be a transient networking or container-startup timing issue.
...
**Cause:** Fresh or empty Neo4j database with no `Event` nodes or `event_id` properties yet. Queries return empty results but do not fail. These warnings disappear once data is populated.
...
**Cause:** Rollout sequences (29125 and 29067 tokens) exceed `max_tokens_per_mb` in the `MicroBatchSpec`. The FFD allocator cannot pack a single sequence larger than the micro-batch capacity.

**Fix:** Increase `max_tokens_per_mb` in `opensage_grpo_mt.yaml` to at least match the maximum possible sequence length (`prompt_len + max_new_tokens`). For agent tasks where prompts can reach ~27K tokens:
...
1. **Generation length too short**: `max_new_tokens=2048` is the total token budget across all turns. Agent tasks require multi-turn tool use (bash, search, analyze), so 2048 tokens total is far too little. The model hits max length before completing the task, producing zero reward.
2. **Batch size too small**: with `n_samples=2` and all-zero rewards, GRPO advantage normalization always yields zero (zero variance → zero advantages).
3. **Rollout bottleneck**: 241s for 2 short sequences suggests the inference server is under-provisioned.

**Fix: recommended parameters for the next training run:**
...
If only 2 GPUs are available, use a minimal improvement config:
...
**Root cause:** `run_agent_in_thread` only captured `part.text` from ADK events, ignoring `part.function_call`. The tool-call parsing chain does work:
...
4. ADK processes the `set_model_response` function call properly.

However, `run_agent_in_thread` missed step 4 because it only looked at text parts.
