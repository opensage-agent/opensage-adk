...
from aigise.extended_features.neo4j_monkey_patch import is_neo4j_logging_enabled
from aigise.utils.agent_utils import (
    discover_all_agents,
    register_callback_to_all_agents,
)

MAX_TOOL_RESPONSE_LENGTH = os.getenv("MAX_TOOL_RESPONSE_LENGTH", 1000)
MAX_HISTORY_SUMMARY_LENGTH = os.getenv("MAX_HISTORY_SUMMARY_LENGTH", 50000)
...
async def tool_response_summarizer_callback(tool, args, tool_context, tool_response):
    ...
    if not hasattr(agent, "canonical_model"):
        print("Agent has no model, skipping summarization")
        return None
    ...
    try:
        ...
        print(f"Error summarizing tool response: {e}")
        tool_response = str(tool_response)
        summary = (
    ...
    if is_neo4j_logging_enabled():
        from aigise.extended_features.neo4j_history_manager import (
            get_neo4j_history_manager,
        )

        neo4j_manager = get_neo4j_history_manager()
        neo4j_manager.create_raw_tool_response_node(
...
async def history_summarizer_callback(tool, args, tool_context, tool_response):
    ...
    if not hasattr(agent, "canonical_model"):
        print("Agent has no model, skipping history summarization")
        return None
    ...
    print(
        f"History text length {total_text_length} exceeds threshold, triggering summarization..."
    )
    ...
    try:
        summary_text = await _summarize_events_async(
            agent.canonical_model, events_to_summarize
        )
    except Exception as e:
    ...
    print(f"History summarized: {len(events_to_summarize)} events → 1 summary event")
    ...
    if is_neo4j_logging_enabled():
        ...
        neo4j_manager = get_neo4j_history_manager()
        neo4j_manager.create_history_summary_node(
