...
def combined_for(
    function_a: Callable, function_b: Callable, name: Optional[str] = None
) -> FunctionTool:
    """
    ...
    def composed_function(*args, **kwargs):
        ...
        if not isinstance(result_a, dict) or "result" not in result_a:
            raise ValueError(
                f"Function {func_a_name} must return dict with 'result' key"
            )
        ...
        if not isinstance(first_element, dict):
            raise ValueError(
                f"Each element in {func_a_name} result list must be a dict"
            )
        ...
        for element in result_list:
            ...
            final_results.append({"source_element": element, "result": result_b})
...
def combined_one(
    ...
    def composed_function(*args, **kwargs):
        ...
        if not isinstance(first_element, dict):
            raise ValueError(
                f"First element in {func_a_name} result list must be a dict"
            )
