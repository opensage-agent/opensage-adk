...
from secagentx.utils.agent_utils import extract_tools_from_agent
...
async def create_subagent(
    ...
    try:
        ...
        if not available_tools:
            return {
                "success": False,
                "error": "No tools available from current agent"
            }
        ...
        config = {
            ...
            "description": description or f"Agent {agent_name} with tools: {', '.join(tools_list)}"
        }
        
        agent_id, agent_instance = await manager.create_agent(
            config,
            creator=current_agent.name
        )
