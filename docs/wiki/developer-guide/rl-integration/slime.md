...
## 2. Install OpenSage Inside the Container
...
Use the OpenSage launcher script (`rl/slime/train.sh`), which handles Docker cleanup, environment setup, and delegation to the SLIME launch script:
...
YAML keys are `train.py` CLI arg names (without `--`). Boolean values of `true` become flags; `false` is skipped.
...
The launcher appends these values as `EXTRA_TRAIN_ARGS` after the script's defaults, so they override matching parameters.
