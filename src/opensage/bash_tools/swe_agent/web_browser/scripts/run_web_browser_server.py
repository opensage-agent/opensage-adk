    ...
    def wrapper(*args, **kwargs):
        if not browser_manager.is_website_open():
            return create_response(
                {"status": "error", "message": "Please open a website first."}, False
            )
...
def _get_response_metadata() -> dict[str, Any]:
    ...
    if browser_manager.is_website_open():
        ...
        metadata["Mouse Position"] = (
            f"({browser_manager.mouse_x},{browser_manager.mouse_y})"
        )
        metadata["Viewport Size"] = (
            f"{browser_manager.window_width}×{browser_manager.window_height}"
        )
        with browser_manager._browser_lock() as page:
            ...
            metadata["Visible Dimensions"] = (
                f"{visible_dims['width']}×{visible_dims['height']}"
            )
    return metadata
...
def drag():
    ...
    if not path or len(path) < 2:
        return create_response(
            {"status": "error", "message": "Path needs at least two points"}, False
        )
    for ix, point in enumerate(path):
        if len(point) != 2:
            return create_response(
                {
                    "status": "error",
                    "message": f"Path point {ix} must have exactly 2 coordinates",
                },
                False,
            )
        x, y = point
...
def keypress():
    ...
    if not isinstance(keys, list):
        return create_response(
            {"status": "error", "message": "Keys must be a list"}, False
        )
    if not keys:
