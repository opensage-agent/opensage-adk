...
def scroll(scroll_x, scroll_y):
    ...
    response = send_request(
        ...
        "POST",
        {
            "scroll_x": scroll_x,
            "scroll_y": scroll_y,
            "return_screenshot": config.autoscreenshot,
        },
    )
    if response is None:
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "scroll_x", type=int, help="The number of pixels to scroll horizontally"
    )
    parser.add_argument(
        "scroll_y", type=int, help="The number of pixels to scroll vertically"
    )
