...
1. **Swap the model.** Most agents default to a model behind our internal LiteLLM proxy. Edit `agent.py` to point `LiteLlm(model=…, api_key=…, base_url=…)` at your provider.
