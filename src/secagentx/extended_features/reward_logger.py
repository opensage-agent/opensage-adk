...
class RewardLogger:
    ...
    def _log_reward(self, session_id: str, reward_type: str, reward_value: float, 
                   message: Optional[str] = None, additional_data: Optional[Dict[str, Any]] = None):
                ...
                if event.content and event.content.parts:
                    for part in event.content.parts:
                        if part.text:
                            ...
                            conversations.append({
                                "from": "function_call",
                                "value": json.dumps(function_call_data, ensure_ascii=False)
                            })
                        elif part.function_response:
                            # Function response as separate conversation item
                            conversations.append({
                                "from": "observation",
                                "value": json.dumps(part.function_response.response, ensure_ascii=False)
                            })
    ...
    def create_after_agent_callback(self):
        ...
        def after_agent_callback(callback_context: CallbackContext) -> Optional[types.Content]:
            ...
            try:
                reward_value = self.reward_function({"response": agent_response}, message)
                ...
                print(f"Error calculating reward for agent {callback_context.agent_name}: {e}")
