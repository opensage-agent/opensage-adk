"""Generate CLI reference pages for OpenSage.

This keeps the docs site in sync with the actual CLI help text without manual
copy/paste. Run before `zensical build`/`zensical serve`:

    uv run python docs/scripts/generate_cli_reference.py

(Zensical, unlike MkDocs, has no build-time hook system, so this is a plain
standalone script rather than something the doc generator invokes itself.)
"""
...
def _annotate_under_development(help_text: str) -> str:
    ...
    """
    help_text = re.sub(r"\bOpenSage\b", "OpenSage", help_text)
    help_text = re.sub(r"\bopensage\b", "opensage", help_text)
...
def _format_dependency_check_paragraph(help_text: str) -> str:
    ...
    which renders poorly in the rendered docs. This rewrites that paragraph
    into multi-line bullets, without changing the actual CLI behavior.
    """
...
def main() -> int:
    # This file lives under docs/scripts/, so parents[2] is the repo root.
    ...
    print(f"Wrote CLI reference pages to {out_dir}")
    return 0
