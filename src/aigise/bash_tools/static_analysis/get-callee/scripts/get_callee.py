...
def _get_callee_helper(client, function_name: str, file_path: Optional[str]) -> dict:
    dict_result = {"result": []}
...
def main():
    parser = argparse.ArgumentParser(description="Get callees of a function.")
    ...
    try:
        ...
        result = _get_callee_helper(client, function_name, file_path)

        # Output as plain text
        if not result["result"]:
            print(f"No callees found for function '{function_name}'.")
        else:
            print(
                f"Found {len(result['result'])} callee(s) for function '{function_name}':\n"
            )
            for i, callee in enumerate(result["result"], 1):
                print(f"Callee {i}:")
                print(f"  Function: {callee['function_name']}")
                print(f"  File: {callee['file_path']}")
                print(f"  Lines: {callee['start_line']}-{callee['end_line']}")
                print(f"  Type: {callee['call_type']}")
                print()
        client.close()

    except Exception as e:
        print(f"Error: Failed to get callees: {str(e)}", file=sys.stderr)
        sys.exit(1)
