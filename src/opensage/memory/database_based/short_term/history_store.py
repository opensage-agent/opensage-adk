...
Provides functions for recording agent execution history, events, and tool
responses in Neo4j.
"""
...
async def record_agent_end(
    ...
    """Record the end of an agent run in Neo4j."""
    client = await get_neo4j_client_from_context(context, "history")
    session_id = context.session.id
...
async def _create_summarize_relation(
    ...
    try:
        ...
        if result:
            ...
            return True
        logger.error("No matching RawToolResponse found for summary content")
        return False
...
async def _maybe_create_summarize_relation(
    ...
    for part in event.content.parts:
        ...
        if (
            ...
            for _, value in response_dict.items():
...
async def create_history_summary_node(
    ...
    try:
        ...
        await client.run_query(create_summary_query, params)
