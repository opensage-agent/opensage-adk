...
class CloneAgentTool(BaseTool):
    ...
    async def call(self, tool_context: ToolContext,
                   source_agent_id: str,
...
class GetAgentInfoTool(BaseTool):
    ...
    async def call(self, tool_context: ToolContext,
        ...
        try:
            ...
            if agent:
                agent_info["current_tools"] = [tool.name for tool in agent.tools] if hasattr(agent, 'tools') else []
                agent_info["sub_agents"] = [sub.name for sub in agent.sub_agents] if agent.sub_agents else []
