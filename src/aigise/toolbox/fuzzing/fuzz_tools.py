...
@requires_sandbox("fuzz")
async def check_fuzzing_coverage(*, tool_context: ToolContext) -> Dict[str, any]:
    """
    ...
    res, exit_code = fuzz_sandbox.run_command_in_container(
        "ls -la /fuzz/out/ 2>/dev/null || echo 'No fuzzing output'"
    )
...
def _parse_crash_output(output: str) -> Dict[str, any]:
    ...
    crash_info = {"type": "unknown", "signal": None, "address": None, "stack_trace": []}
